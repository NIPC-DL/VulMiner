...
class Trainer:
    ...
    def __init__(self, root):
        self._root = utils.path_check(root)
        self._models = []
        self._metrics = {}
        self._events = {
            Events.STARTED: [],
    ...
    def fit(self, folds=None, save=True):
        ...
        """
        path = utils.path_check(self._root + 'ModelSave/')
        ...
        for model in self._models:
            ...
            if folds and isinstance(folds, int):  # use n-folds validation
                ...
                if save:  # save the trained model
                    torch.save(nn.state_dict(),
                               path + f'{nn.__class__.__name__}_nn.pt')
